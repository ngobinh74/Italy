...
void Vector_Cross_Product(float out[3], const float v1[3], const float v2[3])  //cross product
{
...
void Vector_Scale(float out[3], const float v[3], float scale) // tich vo huong, khac voi dot product
{
  for(int c = 0; c < 3; c++)
  {
    out[c] = v[c] * scale; 
